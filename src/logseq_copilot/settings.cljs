...
(defn show-message
  "Shows a notification message in the Logseq UI.
   Parameters:
   - text: Message text to display
   - type: Message type (success, warning, error)"
  [text type]
...
(defn verify-api-key
  "Verifies the API connection by making a test request.
   Checks if the endpoint, key, and model are properly configured.
...
(defn initialize-settings
  "Initializes plugin settings by loading saved configurations.
...
(defn handle-verify
  "Handles the verification button click event.
   Attempts to verify the API connection and updates UI accordingly."
  []
  (-> (verify-api-key)
      (.catch (fn [err]
...
(defn handle-settings-changed
  "Handles settings changes in the plugin configuration.
   Parameters:
   - new-settings: Updated settings object
   - old-settings: Previous settings object
   Updates internal settings state and triggers verification when needed."
